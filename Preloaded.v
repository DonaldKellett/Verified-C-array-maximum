...
Definition _m : ident := 56%positive.
Definition _main : ident := 59%positive.
Definition _maxarray : ident := 57%positive.
Definition _myarr : ident := 58%positive.
Definition _n : ident := 54%positive.
Definition _t'1 : ident := 60%positive.

Definition f_maxarray := {|
  fn_return := tuint;
...
Definition v_myarr := {|
  ...
  gvar_init := (Init_int32 (Int.repr 1) :: Init_int32 (Int.repr 4) ::
                Init_int32 (Int.repr 2) :: Init_int32 (Int.repr 3) :: nil);
  gvar_readonly := false;
  gvar_volatile := false
|}.
...
Definition f_main := {|
  ...
  fn_params := nil;
  fn_vars := nil;
  fn_temps := ((_m, tuint) :: (_t'1, tuint) :: nil);
  fn_body :=
(Ssequence
  (Ssequence
    (Ssequence
      (Scall (Some _t'1)
        (Evar _maxarray (Tfunction (Tcons (tptr tuint) (Tcons tint Tnil))
                          tuint cc_default))
        ((Evar _myarr (tarray tuint 4)) :: (Econst_int (Int.repr 4) tint) ::
         nil))
      (Sset _m (Etempvar _t'1 tuint)))
    (Sreturn (Some (Ecast (Etempvar _m tuint) tint))))
...
((___builtin_ais_annot,
 ...
 (___builtin_debug,
   Gfun(External (EF_external "__builtin_debug"
     ...
     {|cc_vararg:=true; cc_unproto:=false; cc_structret:=false|})) ::
 (_maxarray, Gfun(Internal f_maxarray)) :: (_myarr, Gvar v_myarr) ::
 (_main, Gfun(Internal f_main)) :: nil).

Definition public_idents : list ident :=
(_main :: _myarr :: _maxarray :: ___builtin_debug ::
...
Definition maxarray_spec : ident * funspec :=
DECLARE _maxarray
 ...
 POST [ tuint ]
  PROP () RETURN (Vint (Int.repr (list_max_Z contents)))
  SEP (data_at sh (tarray tuint size) (map Vint (map Int.repr contents)) a).
...
Definition main_spec :=
 DECLARE _main
  ...
  POST [ tint ]
     PROP()
     RETURN (Vint (Int.repr (4)))
     SEP(TT).

Definition Gprog := [maxarray_spec; main_spec].