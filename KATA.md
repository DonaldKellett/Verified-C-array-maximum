...
Suggested reading: [Introduction to Verifiable C](https://softwarefoundations.cis.upenn.edu/vc-current/Verif_sumarray.html)

Suggested warm-up exercise: [Verified list maximum](https://www.codewars.com/kata/60b49f44080926003a20bbed)

## Task

Consider the following C program `maxarray.c`:
...
The function `unsigned maxarray(unsigned *, int)` computes the maximum value in the given array with the given length, and the `main()` function simply invokes `maxarray()` to compute and return the maximum value in `{1, 4, 2, 3}` (which is obviously `4`). In this Kata, we will use [VST](https://vst.cs.princeton.edu/) to formalize a specification for this C program and prove it correct.

### Our functional model

The first step in verifying a C program is to relate it to some functional (mathematical) model and prove properties of that model. Here, we define a fixpoint `list_max_Z` in Coq analogous to `maxarray()` in C which computes the maximum integer in the given list by performing a right fold starting from `0`:

```coq
Definition list_max_Z : list Z -> Z := fold_right Z.max 0.
```

For an empty list, `list_max_Z` returns `0` which makes sense (after all, we must return _something_) and matches the behavior of our `maxarray()` function. However, for nonempty lists containing only negative integers, `list_max_Z` erroneously returns `0` instead of the maximum integer in the list. But notice that `maxarray()` works on an array of unsigned integers and `list_max_Z` serves as a model for `maxarray()`. So we only care about the behavior of `list_max_Z` for lists of non-negative integers, and under this assumption, it satisfies the following properties:

1. `list_max_Z` returns `0` for an empty list
1. Otherwise, no element in the list exceeds the maximum returned by `list_max_Z` and the returned maximum must appear in the list

### Tying the functional model to our C program

The specification for `maxarray()` is given as follows:
...
Very roughly, this states, "Given an array `a` of size at least `n` whose in-memory contents for the first `n` items are modelled by the list `contents` of length `size`, the return value of `maxarray()` is an unsigned integer corresponding to the maximum value in `contents` as defined by `list_max_Z`, and the contents under `a` are left intact."

For completeness, we also provide a spec for `main()`, which basically says that it must return `4` (the maximum of `{1, 4, 2, 3}`):
